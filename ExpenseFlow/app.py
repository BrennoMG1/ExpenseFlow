...
BASE_API_URL = f"{ORCHESTRATOR_URL}/{UIPATH_ORG}/{UIPATH_TENANT}/orchestrator_"
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            resp_buckets = await client.get(
                f"{BASE_API_URL}/odata/StorageBuckets",
                headers=headers,
                params={
...
async def _obter_excel_bytes() -> bytes:
    ...
    if EXCEL_PATH.exists():
        log.info(f"[local] Lendo arquivo local: {EXCEL_PATH}")
